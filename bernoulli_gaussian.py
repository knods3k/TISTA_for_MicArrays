...
def bg_generator(A,pnz=0.01,SNR=40,noise=True):
    ...
    y = np.matmul(A,x)
    noise_var = (10 ** (-SNR/10)) * np.mean(np.abs(y))**2 * 1.825 #(N/M)**(pnz**(M/N))
    y = y + noise*np.random.normal(0,np.sqrt(noise_var),M)
...
for SNR in [60,40,20,0]:    
    for filename in filenames:
        ...
        signal = tf.reduce_mean(tf.square(y_true)).numpy()
        noise = tf.reduce_mean(tf.square(y_true - y)).numpy()
        ratio = signal / noise
        log = 10*np.log10(ratio)
        print(filename,SNR,log)
